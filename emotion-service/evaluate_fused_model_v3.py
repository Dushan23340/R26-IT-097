"""v3 of evaluate_fused_model.py - evaluates model/best_fused_model_v3 on
its own held-out split (dataset/facial_features_v3.npz, same
random_state/stratify used at training time, so no leakage).

Unlike v2's evaluation, this also reports accuracy on a SYNTHETICALLY
OCCLUDED copy of the same validation set (a fixed 25%-of-crop rectangular
blank, applied deterministically so the comparison is reproducible) -
per the proposal's validation strategy (3.5): "additional evaluation will
be conducted using synthetically occluded test samples". Comparing clean
vs. occluded accuracy on the SAME model is the direct empirical check that
the occlusion-handling mechanisms (Cutout augmentation + region weighting)
actually help, not just that they exist in the training code.
"""
...
VAL_SPLIT = 0.2
SEED = 42
# Fixed (not random) occlusion for reproducible clean-vs-occluded comparison:
# a centered square covering the middle of the face - big enough to hide a
# meaningful chunk of one of the eye/mouth bands, small enough that a human
# could still plausibly recognize the expression from what remains.
OCCLUSION_FRAC = 0.30
...
print("Loading model/best_fused_model_v3.keras")
model = load_model("model/best_fused_model_v3.keras")
y_true = val_labels
...
for label, occluded in (("CLEAN", False), ("SYNTHETICALLY OCCLUDED (30% center blank)", True)):
    ...
    if not occluded:
        ...
        plt.ylabel("Actual")
        plt.tight_layout()
        plt.savefig("model/fused_confusion_matrix_v3.png")
        print("Saved confusion matrix to model/fused_confusion_matrix_v3.png")
