"""Evaluates model/best_custom_cnn_v4 on its own held-out split
(dataset/facial_features_v3.npz, same random_state/stratify used at
training time, so no leakage) - same methodology as
evaluate_fused_model_v3.py, so the two are directly comparable.

Unlike v3's evaluation, no preprocess_input here (that's MobileNetV2/
ImageNet-specific normalisation) - the model's own Rescaling(1/255) layer
handles it, so raw 0-255 pixel values are passed straight through, exactly
matching train_custom_cnn_v4.py's pipeline.
"""
...
IMG_SIZE = 48  # matches train_custom_cnn_v4.py / proposal 3.2
BATCH_SIZE = 32
VAL_SPLIT = 0.2
SEED = 42
# Same fixed 30% center blank as evaluate_fused_model_v3.py, for a direct
# apples-to-apples clean-vs-occluded comparison against v3's numbers.
OCCLUSION_FRAC = 0.30
...
for label, occluded in (("CLEAN", False), ("SYNTHETICALLY OCCLUDED (30% center blank)", True)):
    ...
    if not occluded:
        ...
        plt.ylabel("Actual")
        plt.tight_layout()
        plt.savefig("model/custom_cnn_confusion_matrix_v4.png")
        print("Saved confusion matrix to model/custom_cnn_confusion_matrix_v4.png")
