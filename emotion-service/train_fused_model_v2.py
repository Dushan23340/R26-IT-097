"""v2 of train_fused_model.py - retrains the model actually served by
flask_api.py (best_fused_model.h5) on the FER-2013 + AffectNet merged
image set (dataset/expression_dataset_v2) instead of FER-2013 alone.
Requires dataset/facial_features_v2.npz (see
scripts/extract_facial_features_v2.py) to already exist.

Same architecture/procedure as train_fused_model.py (kept unmodified for a
clean before/after comparison) - only the features file and output
filenames change, so the current production model
(model/best_fused_model.h5) is never overwritten by this script. Swap it in
manually only after evaluate_fused_model_v2.py shows it's actually better.
"""
...
checkpoint = ModelCheckpoint(
    "model/best_fused_model_v2.keras", monitor="val_accuracy", save_best_only=True, mode="max", verbose=1
)
...
model.save("model/fused_model_v2_final.keras")
model.save("model/best_fused_model_v2.h5")

with open("model/fused_class_indices_v2.json", "w", encoding="utf-8") as f:
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("Fused Model Training Accuracy (v2: FER-2013 + AffectNet)")
plt.legend()
plt.savefig("model/fused_training_accuracy_v2.png")
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Fused Model Training Loss (v2: FER-2013 + AffectNet)")
plt.legend()
plt.savefig("model/fused_training_loss_v2.png")

print("Training graphs saved!")
print("Best model saved as: model/best_fused_model_v2.keras / model/best_fused_model_v2.h5")
