"""v2 of extract_facial_features.py - same MediaPipe blendshape/head-pose
extraction, pointed at dataset/expression_dataset_v2 (FER-2013 + AffectNet
merged, see build_expression_dataset_v2.py) instead of the FER-2013-only
dataset/expression_dataset. Feeds train_fused_model_v2.py, which retrains
the model actually served by flask_api.py (best_fused_model.h5) - the
plain-CNN retrain (train_mobilenetv2_v2.py) already done separately isn't
what's live.
...
DATASET_DIR = SERVICE_DIR / "dataset" / "expression_dataset_v2"
MODEL_PATH = SERVICE_DIR / "model" / "mediapipe" / "face_landmarker.task"
OUTPUT_PATH = SERVICE_DIR / "dataset" / "facial_features_v2.npz"
