"""v4 of train_fused_model.py - one change over v3: minority-class
oversampling on top of the existing "balanced" class_weight.

Diagnosed via scripts run against the real, live inference pipeline (not
just held-out validation) that v3's model (best_fused_model.h5) classifies
Frustrated at only ~45% accuracy - 55% of real Frustrated faces get called
Bored or Confused instead, often with high (>0.9) confidence, i.e. not a
borderline call. dataset/facial_features_v3.npz's label distribution shows
why: Frustrated has 1,780 samples against Bored's 22,368 (12.5x fewer) -
sklearn's class_weight="balanced" (kept below, unchanged) reweights the
LOSS per class but a rare class is still only seen ~1,780 times in an
epoch's worth of distinct images, right or wrong. Oversampling (repeating
indices before the tf.data pipeline, so each repeat still gets independent
random augmentation via _augment()'s tf.random ops on every draw) increases
how many times the model actually SEES the minority class's visual patterns
per epoch, which loss reweighting alone doesn't fix.

Oversampling rule: any class under 30% of the majority class's TRAIN count
gets its indices repeated up to that 30% target, capped at 3x its own
original count (avoids overfitting a very small class through excessive
verbatim repetition - Frustrated's real fix is more data, this narrows the
gap without pretending 1,780 images are 17,894). class_weight is then
recomputed on the POST-oversampling distribution, so it applies a much
milder residual correction than v3's, rather than compounding two full
corrections on top of each other.

Same architecture/procedure/augmentation/occlusion-handling as
train_fused_model_v3.py (kept unmodified for a clean before/after
comparison) - only the oversampling step and output filenames change. The
...
checkpoint = ModelCheckpoint(
    "model/best_fused_model_v4.keras", monitor="val_accuracy", save_best_only=True, mode="max", verbose=1
)
...
model.save("model/fused_model_v4_final.keras")
model.save("model/best_fused_model_v4.h5")

with open("model/fused_class_indices_v4.json", "w", encoding="utf-8") as f:
...
plt.xlabel("Epoch")
plt.ylabel("Accuracy")
plt.title("Fused Model Training Accuracy (v4: 6-class + occlusion handling + oversampling)")
plt.legend()
plt.savefig("model/fused_training_accuracy_v4.png")
...
plt.xlabel("Epoch")
plt.ylabel("Loss")
plt.title("Fused Model Training Loss (v4: 6-class + occlusion handling + oversampling)")
plt.legend()
plt.savefig("model/fused_training_loss_v4.png")

print("Training graphs saved!")
print("Best model saved as: model/best_fused_model_v4.keras / model/best_fused_model_v4.h5")
