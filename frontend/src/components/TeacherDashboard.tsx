...
export function TeacherDashboard() {
  ...
  return (
    <div className="space-y-6 stagger-children max-w-7xl mx-auto">
      {/* Alerts Panel */}
      {showAlerts && (
        <div className="fixed top-20 right-6 z-50 w-96 glass rounded-2xl shadow-2xl border border-border/60">
          ...
          <div className="max-h-96 overflow-y-auto">
            {alerts.map((alert) => (
              <div key={alert.id} className="p-4 border-b border-border/40 last:border-b-0">
                <div className="flex gap-3">
                  {alert.type === "danger" && (
                    <AlertTriangle className="h-5 w-5 text-emotion-angry flex-shrink-0 mt-0.5" />
                  )}
                  {alert.type === "warning" && (
                    <AlertTriangle className="h-5 w-5 text-emotion-confused flex-shrink-0 mt-0.5" />
                  )}
                  {alert.type === "info" && (
      ...
      <div className="glass rounded-2xl p-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="flex-1">
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium ${
                isLive ? "bg-destructive/10 text-destructive" : "bg-secondary text-muted-foreground"
              }`}>
                ...
                <>
                  <button
                    ...
                    className={`px-5 py-3 rounded-xl font-semibold text-sm flex items-center gap-2 transition-all hover:scale-105 ${
                      isSharingScreen ? "bg-primary/20 text-primary" : "bg-secondary text-foreground"
                    }`}
                  >
          ...
          >
            ...
            {alerts.length > 0 && (
              <div className="absolute top-2 right-2 h-3 w-3 rounded-full bg-destructive border-2 border-background" />
            )}
          </button>
        </div>
      </div>
      ...
      <div className="glass rounded-2xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          {Object.entries(emotionDistribution).map(([emotion, percentage]) => {
            const emotionKey = emotion as EmotionKey;
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="space-y-2">
            {classStats.weakTopics.map((topic, index) => (
              <div key={index} className="flex items-center gap-2 p-2 rounded-lg bg-destructive/5 text-sm">
            ...
            return (
              <div 
                ...
                className={`p-3 rounded-xl border transition-all hover:scale-105 ${
                  student.status === "inactive" ? "opacity-50" : ""
                }`}
                style={{ 
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className={`h-2 w-2 rounded-full ${
                    student.status === "active" ? "bg-emotion-happy" : "bg-muted-foreground"
                  }`} />
                </div>
      ...
      <div className="glass rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button className="px-4 py-2 rounded-lg text-sm font-semibold text-primary-foreground flex items-center gap-2 transition-all hover:scale-105"
        ...
        <div className="space-y-3">
          {upcomingClasses.map((cls) => (
            <div key={cls.id} className="flex items-center justify-between p-4 rounded-xl border border-border/60 hover:border-primary/40 transition-colors">
              <div className="flex-1">
