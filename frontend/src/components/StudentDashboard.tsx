...
export function StudentDashboard() {
  ...
  useEffect(() => {
    const emotions: EmotionKey[] = ["happy", "neutral", "confused", "happy", "neutral"];
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                {notifications.length > 0 && (
                  <div className="absolute top-1 right-1 h-2.5 w-2.5 rounded-full bg-primary border-2 border-background" />
                )}
              </button>
            </div>
          </div>
  ...
  // Main Dashboard
  return (
    <div className="space-y-6 stagger-children max-w-7xl mx-auto">
      {/* Notifications Panel */}
      {showNotifications && (
        <div className="fixed top-20 right-6 z-50 w-80 glass rounded-2xl shadow-2xl border border-border/60">
          ...
          <div className="max-h-96 overflow-y-auto">
            {notifications.map((notif) => (
              <div key={notif.id} className="p-4 border-b border-border/40 last:border-b-0 hover:bg-secondary/30 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass rounded-2xl p-6">
          ...
          {liveClasses.filter(c => c.status === "live").map((cls) => (
            <div key={cls.id} className="mb-4 p-4 rounded-xl border-2 border-primary/30 bg-primary/5">
          ...
          <div>
            ...
            <div className="space-y-2">
              {liveClasses.filter(c => c.status === "upcoming").map((cls) => (
                <div key={cls.id} className="flex items-center justify-between p-3 rounded-lg border border-border/60 hover:border-primary/30 transition-colors">
                  <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {progressData.map((item) => (
            <div key={item.subject} 
              ...
              {item.weak && (
                <p className="text-xs text-destructive font-medium">Needs Practice</p>
              )}
            </div>
          ))}
        </div>
      ...
      <div className="glass rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {recommendations.map((rec) => (
            <div key={rec.id} className="p-4 rounded-xl border border-border/60 hover:border-primary/40 transition-all hover:shadow-lg">
      ...
      <div className="glass rounded-2xl p-6">
        ...
        <div className="space-y-3">
          {quizzes.map((quiz) => (
            <div key={quiz.id} className="flex items-center justify-between p-4 rounded-xl border border-border/60 hover:border-primary/40 transition-all">
              <div className="flex-1">
              ...
              <div className="flex items-center gap-4">
                <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                  quiz.difficulty === "Easy" ? "bg-emotion-happy/10 text-emotion-happy" :
                  quiz.difficulty === "Medium" ? "bg-emotion-confused/10 text-emotion-confused" :
                  "bg-emotion-angry/10 text-emotion-angry"
                }`}>
                  {quiz.difficulty}
                </span>
                <button className="px-5 py-2 rounded-lg text-sm font-semibold text-primary-foreground transition-all hover:scale-105"
