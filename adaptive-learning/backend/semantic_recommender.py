...
from data import LO_DESCRIPTIONS, RESOURCES
...
def recommend_resources(lo_name: str, emotion: str | None = None, top_k: int = 3) -> list[dict]:
    """Semantic-similarity ranking of resources for one LO, re-weighted by
    emotional state per the proposal's modulation rules."""
    candidates = RESOURCES.get(lo_name, [])
    if not candidates:
        return []
